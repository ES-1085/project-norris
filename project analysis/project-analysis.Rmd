...
The large majority of facilities in this dataset are located in North America, but a significant number of U.S.-owned facilities are strewn across the globe in other territories -- possibly shifting any inferences that can be made on the state of pollution solely in the U.S.
...
The percentage of facilities on tribal land is less than 1%, but this does not make it negligible as the impact still matters.
...
Using these tables, we have set up a groundwork map for chemical usage, which leads us to ask: knowing the drastic difference in percent of facilities on tribal land or not, does the location of tribal land have any effect on the type and amount of chemical release when compared to facilities not on tribal land?

We can take points from history and U.S. law, knowing that tribes of the American West often do have significant health hazards due to the many power plants, factories, mines, waste dumps, and testing sites that are placed on their land -- sometimes with permission as a means to acquiring much-needed funds, and sometimes out of a tribe's power.

## Industry Analysis

Within these questions of location, there is the question of each facility's purpose. Condensing the purpose of a factory into just 11 categories, their individual patterns of location, chemical use, and pollution can be tracked. This prevalence brings up the question: is there a correlation between an industry's sector and their frequency of chemicals? Further, their frequency of specific types of chemicals -- carcinogens, PFAS', and PBT'? 
...
The most common purpose of a U.S. facility is chemical production, and the least common is apparel production which makes sense given trends in American imports -- though all clearly produce chemicals as waste as well.

```{r industry-sector-distribution, echo = FALSE, message = FALSE, warning = FALSE}
tri_data %>% 
...
Fitting with the most common industry sector, the chemical-producing facilities also use the most carcinogens (non-comprehensive), but the frequency seems to [almost] consistently grow with the prevalence of these sectors.
...
```{r chemical-trends-by-category-elemental-metal-included, echo = FALSE, message = FALSE, warning = FALSE}
tri_data %>% 
...
```{r chemical-trends-by-category-clean-air-act, echo = FALSE, message = FALSE, warning = FALSE}
tri_data %>% 
  select(year, elemental_metal_included, clean_air_act_chemical, metal, metal_category, carcinogen, pbt, pfas) %>% 
  distinct(year, elemental_metal_included, clean_air_act_chemical, metal, metal_category, carcinogen, pbt, pfas) %>% 
...
```{r chemical-trends-by-category-metal, echo = FALSE, message = FALSE, warning = FALSE}
tri_data %>% 
  select(year, elemental_metal_included, clean_air_act_chemical, metal, metal_category, carcinogen, pbt, pfas) %>% 
  distinct(year, elemental_metal_included, clean_air_act_chemical, metal, metal_category, carcinogen, pbt, pfas) %>% 
...
```{r chemical-trends-by-category-metal-category, echo = FALSE, message = FALSE, warning = FALSE}
tri_data %>% 
  select(year, elemental_metal_included, clean_air_act_chemical, metal, metal_category, carcinogen, pbt, pfas) %>% 
  distinct(year, elemental_metal_included, clean_air_act_chemical, metal, metal_category, carcinogen, pbt, pfas) %>% 
...
```{r chemical-trends-by-category-carcinogen, echo = FALSE, message = FALSE, warning = FALSE}
tri_data %>% 
  select(year, elemental_metal_included, clean_air_act_chemical, metal, metal_category, carcinogen, pbt, pfas) %>% 
  distinct(year, elemental_metal_included, clean_air_act_chemical, metal, metal_category, carcinogen, pbt, pfas) %>% 
...
From these observed chemicals, there is still the question of their disposal after use. With air, ground, and water being the possible points of disposal, we must assess the trends in these differing types of waste removal. In the past decade, what are these trends in pollution due to chemical release, whether on-site or off?
...
On-site release types: variables `Underground`, `Landfill`, and `Surace Impoundment` are all zero.
...
Pollution variable `Releases` is all zero.
...
```{r off-site-energy-recovery-total-by-chemical, echo = FALSE, message = FALSE, warning = FALSE}
chemical_analysis_data %>% 
...
```{r off-site-treated-total-by-chemical, echo = FALSE, message = FALSE, warning = FALSE}
chemical_analysis_data %>% 
...
chemical_analysis_data %>% 
  ggplot(aes(x = off_site_total, y = off_site_energy_recovery_total)) +
  geom_point(aes(alpha = 0.3, color = chemical)) +
  labs(title = "Total Energy Recovered and Off Site Total",
       x = "Off Site Total",
       y = "Total Energy Recovered",
       color = "Chemical",
       alpha = "Saturation") +
  theme_minimal() +
  scale_color_viridis_d() +
...
Code wouldn't run on Joanna's computer, try on Cedar's?
