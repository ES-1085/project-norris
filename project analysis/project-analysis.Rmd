...
chemical_analysis_data %>% 
  ...
  distinct(longitude, latitude, facility_name, parent_co_name, industry_sector, city, state, chemical) %>% 
  leaflet() %>% 
  addTiles() %>% 
    addCircleMarkers(lng = ~longitude, 
                 lat = ~latitude, 
                 clusterOptions = markerClusterOptions(),
                 popup = ~paste0(facility_name, "<br>", parent_co_name, "<br>", industry_sector, "<br>", chemical, "<br>", city, ", ", state))
```
...
```{r}
chemical_analysis_data %>% 
  ggplot(aes(x = on_site_total, color = chemical, fill = chemical, alpha = 0.3)) +
  geom_histogram(position = "dodge", binwidth = 75) +
  scale_color_viridis_d() +
  scale_fill_viridis_d()
``` 
...
>>>>>>> 6a5006feb32baa4a51203e82b2eaad5b3412b794
